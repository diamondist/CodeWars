...
# and of the new one decrease of 1.5 percent per month. Furthermore this
...
#
# parameter (positive int or float, guaranteed) startPriceOld (Old car
# price)
# parameter (positive int or float, guaranteed) startPriceNew (New car
# price)
...
# car and 766 is the nearest integer to 766.158... (rounding 766.158
# gives 766).
#
# Note:
#
...
def nbMonths(price_old, price_new,
             saving_per_month, percent_loss_by_month):
    month = 1
    savings = 0
    total = price_old
    while total < price_new:
        month += 1
        price_old -= price_old * percent_loss_by_month / 100
        price_new -= price_new * percent_loss_by_month /100
        if month % 2 == 0:
            percent_loss_by_month += 0.5
        savings += saving_per_month
        total = price_old + savings
    return [month - 1, round(total - price_new)]
