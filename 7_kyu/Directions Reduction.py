...
#
# [North, South, South, East, West, North, West]
# You can immediatly see that going "NORTH" and immediately "SOUTH" is not reasonable, better
...
def dirReduc(arr):
    exceptions = [('NORTH', 'SOUTH'), ('SOUTH', 'NORTH'), ('EAST', 'WEST'), ('WEST', 'EAST')]
    modified = True
    while modified:
        modified = False
        for i, el in enumerate(arr[:-1]):
            if (arr[i], arr[i + 1]) in exceptions:
                del arr[i]
                del arr[i]
                modified = True
                break
